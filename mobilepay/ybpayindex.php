...
$yeepay = new yeepayMPay($merchantaccount,$merchantPublicKey,$merchantPrivateKey,$yeepayPublicKey);
	...
	//GET得到订单号
	if($_GET['orderid'])
		$order_id 	= $_GET['orderid'];//订单号由后台生成并反馈给前端
	else 
	...
	$product_id = '0001';
	$user_ip = '115.29.42.184'; 
	$transtime 	= time();                                                          //交易时间
	$product_catalog ='1';   							//商品类别码
	$identity_id = '2344442err233';           			//支付身份标识
	$identity_type = 0;     							//支付身份标识类型码
    
    $callbackurl ='http://115.29.42.184/ugcpay/mobilepay/callback.php';//'http://115.29.42.184/ugcpay/zone.php?type=paycallback&userid='.$user_id.'&productid＝'.$product_id.'&orderid＝'.$order_id; //商户后台系统回调地址，前后台的回调结果一样
    //echo $callbackurl;
	
    $fcallbackurl = "";           //商户前台系统回调地址，前后台的回调结果一样
	$product_name = '暗影之枪1000金币';        				//商品名称
	$product_desc = '1000金币';        						//商品描述
